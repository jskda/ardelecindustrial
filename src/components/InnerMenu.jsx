...
export default function MainMenu() {
  ...
  const items = [...menuItems, ...socialUrls];
  ...
  return (
    <animated.nav className="bg-white shadow-md sticky top-0 z-50 bg-opacity-40">
        ...
        <div className="flex items-center ml-6 w-full justify-end space-x-6">
          {trail.map((style, index) => {
            if (index < menuItems.length) {
              ...
              const socialIndex = index - menuItems.length;
              return (
                <animated.div key={socialIndex} style={style}>
                  <SocialIcon
                    url={socialUrls[socialIndex]}
                    target="_blank"
                    rel="noopener noreferrer"
                    title={`Social Icon ${socialIndex}`}
                    fgColor="gray"
                    bgColor="transparent"
